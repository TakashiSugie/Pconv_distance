...
class PConvUnet(object):
    ...
    def loss_hole(self, mask, y_true, y_pred):
        ...
        #print(type(mask))
        #print(mask.numpy())
        mask_np=tf.Session().run(mask)
        true_np=tf.Session().run(y_ture)
        pred_np=tf.Session().run(y_pred)
        #mask_np=tf.Session().run(mask)
        
        mask_inv=1-mask
        distance=cv2.distanceTransform(mask_inv,cv2.DIST_L2,5)
        distnace_inv=1-distance
        return self.l1((1-mask) * y_true*distance_inv, (1-mask) * y_pred*distance_inv)#これにdistanceを考慮する
