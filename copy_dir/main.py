...
tf.keras.backend.set_session(tf.Session(config=config))#なんかこれはよい



from libs.pconv_model_distance_ver2 import PConvUnet
from libs.util import MaskGenerator
import libs.my_img as my_img
...
def parse_args():
    ...
    parser.add_argument(
        '-vgg_path', '--vgg_path',
        type=str, default='pytorch_to_keras_vgg16.h5',
...
class AugmentingDataGenerator(ImageDataGenerator):
    ...
    def flow_from_directory(self, directory, mask_generator, *args, **kwargs):
        ...
        while True:
            ...
            distance_1ch=cv2.distanceTransform(255-255*mask[0,:,:,0],cv2.DIST_L2,5)
            distance_1ch=1-distance_1ch/255
            distance_1ch=np.reshape(distance_1ch,(1,512,512,1))
            #print(distance_1ch.shape)
            distance=np.concatenate((distance_1ch, distance_1ch,distance_1ch), axis=3)
            #distance=1-distance/255
            #print(distance.shape)
            #cv2.imshow("mask",255-255*mask[0,:,:,:])
            #cv2.imshow("distance",distance[0,:,:,:])
            #cv2.waitKey()
            # Yield ([ori, masl],  ori) training batches
            # print(masked.shape, ori.shape)
            gc.collect()
            yield [masked, mask, distance], ori
...
if __name__ == '__main__':
    ...
    (masked, mask,distance), ori = test_data
    ...
    def plot_callback(model, path):
        ...
        for i in range(len(ori)):
            ...
            plt.close()
            """
            cv2.imwrite("./data/test/%s_masked.png"%pred_time,masked[i,:,:,:]*255)
            cv2.imwrite("./data/test/%s_pred.png"%pred_time,pred_img[i,:,:,:]*255)
            cv2.imwrite("./data/test/%s_ori.png"%pred_time,ori[0,:,:,:]*255)
            #cv2.imwrite("./data/test/%s_pred_gt.png"%pred_time,pred_gt*255)
            print("save_now...")
