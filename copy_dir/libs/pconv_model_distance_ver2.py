...
class PConvUnet(object):
    ...
    def __init__(self, img_rows=512, img_cols=512, vgg_weights="imagenet", inference_only=False, net_name='default', gpus=1, vgg_device=None):
        ...
        if self.gpus <= 1:
            self.model, inputs_mask ,inputs_distance= self.build_pconv_unet()
            self.compile_pconv_unet(self.model, inputs_mask,inputs_distance)            
        else:
    ...
    def build_pconv_unet(self, train_bn=True):      
        ...
        inputs_distance = Input((self.img_rows, self.img_cols, 3), name='inputs_distance')
        ...
        model = Model(inputs=[inputs_img, inputs_mask,inputs_distance], outputs=outputs)

        return model, inputs_mask,inputs_distance

    def compile_pconv_unet(self, model, inputs_mask,inputs_distance, lr=0.0002):
        model.compile(
            optimizer = Adam(lr=lr),
            loss=self.loss_total(inputs_mask,inputs_distance),
            metrics=[self.PSNR]
        )


    def loss_total(self, mask,distance):
        def loss(y_true, y_pred):
            #print(mask)
            l2 = self.loss_hole(mask,distance ,y_true, y_pred)
            return 60*l2

        return loss

    def loss_hole(self, mask,distance, y_true, y_pred):
        #やってることは通常の関数と同じで、画像を3枚入力すると、そのL1距離を返してくれる、返す値は浮動小数
        """Pixel L1 loss within the hole / mask"""
        return self.l1((1-mask) * y_true*distance, (1-mask) * y_pred*distance)#1-maskでホール領域白
    ...
    def loss_hole(self, mask, y_true, y_pred):
        #やってることは通常の関数と同じで、画像を3枚入力すると、そのL1距離を返してくれる、返す値は浮動小数
        #Pixel L1 loss within the hole / mask
        return self.l1((1-mask) * y_true, (1-mask) * y_pred)#1-maskでホール領域白
    """
    ...
    def load(self, filepath, train_bn=True, lr=0.0002):
        ...
        # Create UNet-like model
        self.model, inputs_mask,inputs_distance = self.build_pconv_unet(train_bn)
        self.compile_pconv_unet(self.model, inputs_mask,inputs_distance, lr) 
