...
class MaskGenerator():
    ...
    def _generate_mask(self):
        ...
        size = int((self.width + self.height) * 0.15)#size
        ...
        for _ in range(randint(1, 5)):
            x1, y1 = randint(int(self.width/4), int(self.width*(3/4))), randint(int(self.height/4), int(self.height*(3/4)))#中心
            s1, s2 = randint(1, self.width/2), randint(1, self.height/2)# 軸
            a1, a2, a3 = randint(3, 180), randint(3, 180), randint(3, 180)# 角度　スタート角度　エンド角度
            thickness = randint(int(size/2), size)# 太さ
            cv2.ellipse(img, (x1,y1), (s1,s2), a1, a2, a3,(1,1,1), thickness)
            #cv2.ellipse(img, center, axes, angle, startAngle, endAngle, color, thickness=1, lineType=cv2.LINE_8, shift=0)
        return 1-img
            
    def _generate_mask2(self):
        """Generates a random irregular mask with lines, circles and elipses"""
    ...
    def mynext(self,width=512,height=512,ch=1):##########################################
        """Generates a random irregular mask with lines, circles and elipses"""
        img = np.zeros((self.height, self.width, self.channels), np.uint8)
        size = int((self.width + self.height) * 0.15)#size
